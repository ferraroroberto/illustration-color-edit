...
- **Config & secrets:** folder paths in `config.json`; color settings in `color-config.json`; secrets always in `.env`, never committed. The canonical name for the env file is `.env` (not `venv` or anything else — `.venv` is the venv directory).
