...
- **Config & secrets:** folder paths in `config.json`; color settings in `color-config.json`; secrets always in `.env`, never committed. The canonical name for the env file is `.env` (not `venv` or anything else â€” `.venv` is the venv directory).
...
- UI code only in `app/`. Data logic stays in `src/`. Never import `streamlit` from `src/`.
- User feedback via `st.error()` / `st.warning()` / `st.success()`, not `st.write()`.
- **App layout:** `app/app.py` handles only page config, shared state, sidebar, and tab routing. Each tab lives in its own file (`app/tab_*.py`) exposing a `render_*` function. Default to `st.tabs()`.
